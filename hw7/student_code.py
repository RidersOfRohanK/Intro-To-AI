...
class LeNet(nn.Module):
    ...
    def forward(self, x):
        shape_dict = {}
        # certain operations
        out = self.p1(self.relu(self.c1(x)))
        shape_dict[0] = (list(out.size()))

        out = self.p2(self.relu(self.c2(out)))
...
def count_model_params():
    ...
    '''
    model = LeNet()
    modelParameters = filter(lambda p: p.requires_grad, model.parameters())
    modelParams = sum([np.prod(p.size()) for p in modelParameters])
    modelParams = modelParams / 1000000
